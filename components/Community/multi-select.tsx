...
const MultiSelect: React.FC<MultiSelectProps> = ({
  ...
  const handleSelect = (option: string) => {
    setSelectedItems((prev) => {
      ...
      // If "All States" is selected, only allow "All States" to be selected
      if (option === "All States") {
        newSelection = prev.includes("All States") ? [] : ["All States"];
      } else {
        // Otherwise, toggle the selected item
        if (prev.includes(option)) {
          newSelection = prev.filter((item) => item !== option);
        } else if (prev.length < maxSelections) {
          newSelection = [
            ...prev.filter((item) => item !== "All States"),
            option,
          ];
        } else {
          newSelection = prev;
