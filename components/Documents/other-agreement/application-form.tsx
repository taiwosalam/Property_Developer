const HeadText = ({ title }: { title: string }) => {
  return (
    <div>
      <h2 className="uppercase text-[#2394f1] mt-3 font-bold text-xl">
        {title}
      </h2>
    </div>
  );
  ...
  return (
    <div className="w-full">
      <div className="w-full items-center justify-center mb-4">
        <h1 className="font-bold text-[#2394f1] text-3xl text-center stroke-texts">
        ...
        <thead>
          <tr>
            ...
            {titles.map((title, index) => (
              <th
                key={index}
                className="font-bold text-xl uppercase border-4 border-[#2394f1] bg-gray-100 text-left py-2 px-4"
              >
                {title}
              </th>
            ))}
