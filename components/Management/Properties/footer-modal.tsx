  ...
  const handleAddClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (addedUnits.length > 0) {
      ...
      for (let i = 0; i < count; i++) {
        const newUnit = { ...lastUnit, id: `temp-${Date.now()}-${i}` };
        addUnit(newUnit);
      }
      setIsOpen(false);
    } else {
...
//       ? props.duplicate
//       : context?.duplicate ?? { val: false, count: 1 };
//   const setDuplicate =
//     props.setDuplicate !== undefined
//       ? props.setDuplicate
//       : context?.setDuplicate ?? (() => {});
//   const submitLoading =
//     props.submitLoading !== undefined
//       ? props.submitLoading
