...
const ProfessionalPlan = ({ page }: { page?: "modal" | "subscription" }) => {
  return (
    <div
    className={clsx(
      "flex flex-col min-w-[400px] w-full pr-10 pl-4 rounded-md mt-8 flex-wrap shadow-md py-4 bg-[url('/icons/enrollment-bg.svg')] bg-no-repeat bg-center bg-cover bg-opacity-60",
      page === "modal" && "h-[465px] justify-between"
    )}
  >
    <div className="flex flex-col gap-4">
      ...
      <h4 className="text-sm font-bold text-text-secondary dark:text-white">
        Features Included:
      </h4>
      <strong className="leading-[150%] max-w-[750px]">
    ...
    <div
      className={clsx(
        "bg-brand-9 h-10 text-white px-4 py-2 rounded-md flex items-center justify-center mt-6",
        page === "modal" ? "w-full" : "w-full sm:w-1/2 lg:w-1/5"
      )}
    >
      <Link
