...
const useGetConversation = (id: string) => {
  ...
  useEffect(() => {
    // Abort the previous connection if it exists
    if (abortControllerRef.current) {
      abortControllerRef.current.abort();
    }

    // Create a new AbortController for the current connection
    const controller = new AbortController();
    ...
    // Cleanup function to abort the connection when the component unmounts or id changes
    return () => {
      controller.abort();
    };
  }, [id, token, data.conversations, setChatData]);
