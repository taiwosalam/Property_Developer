...
//   resetTour: (tourKey: string) => void;
//   goToStep: (stepIndex: number) => void;
//   startTour: (pathname: string) => void;
// }
...
//     if (get().persist && tourKey) {
//       saveLocalStorage(`hasCompleted${tourKey}`, JSON.stringify(true));
//     }
//     set({
//       tour: { run: false, stepIndex: 0, steps: [], tourKey: "" },
//       shouldRenderTour: false,
...
//     const value = getLocalStorage(`hasCompleted${tourKey}`);
//     try {
...
//   resetTour: (tourKey) => {
//     if (tourKey) {
//       saveLocalStorage(`hasCompleted${tourKey}`, JSON.stringify(false));
//     }
//     set({
//       tour: { run: false, stepIndex: 0, steps: [], tourKey: "" },
//       shouldRenderTour: true,
...
//           !get().isTourCompleted(config.tourKey)
//         ) {
//           get().setPersist(true); // Default to persistent for other tours
//           return config;
//         }
//       }
//       return { steps: [], tourKey: "" };
...
//     const config = selectTourConfig();
//     if (!config.tourKey || config.steps.length === 0) {
//       console.warn("No valid tour configuration found to start.");
//       return;
//     }
//     const steps = getTourStepsWithWelcome(config);
//     const validSteps = checkTargets(steps);
...
//       });
//     } else {
//       console.warn("No valid steps available to start tour.");
//     }
//   },
// }));
