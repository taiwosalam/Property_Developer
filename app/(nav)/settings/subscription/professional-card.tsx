...
const ProfessionalPlanCard = ({
  ...
  const price = "CONTACT FOR PRICE";
  const lifetimePrice = "CONTACT FOR PRICE";
  const page = "settings"; // This should trigger modal behavior

  // Mock onSelectPlan to handle plan selection (can be replaced with actual logic)
  const onSelectPlan = async () => {
    // For professional plan, this might just open the callback modal
    // The actual logic will be handled in the modal
    return true;
  };

  // Mock onSelect for direct selection
  const onSelect = async () => {
    // For professional plan, this might trigger a callback request
    return true;
  };
  ...
  return (
    ...
    >
      ...
      {/* Use SelectPlanButton with page="settings" to trigger modal */}
      <SelectPlanButton
        isFree={false}
        price={price}
        planTitle={planTitle}
