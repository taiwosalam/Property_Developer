...
import { ComplainDetailsPageData, transformComplainDetailsData } from "./data";
...
const PreviewComplain = () => {
  ...
  const [pageData, setPageData] = useState<ComplainDetailsPageData | null>(
    null
  );
  ...
  const {
    ...
    isNetworkError,
    refetch,
  } = useFetch<ComplainResponseDetails>(`complains/${complainId}`);

  useRefetchOnEvent("complainDispatch", () => refetch({ silent: true }));

  useEffect(() => {
    if (apiData) {
      const transformData = transformComplainDetailsData(apiData);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-2 max-md:pt-4">
        ...
        <Button
          href={`/tasks/complains/${complainId}/manage`}
          size="sm_medium"
          className="py-2 px-3"
        >
          manage complain
