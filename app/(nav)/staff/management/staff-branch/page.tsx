...
const BranchStaffPage = () => {
  ...
  const handleSelectTableItem = (item: DataItem) => {
    router.push(
      `/management/staff-branch/${BRANCH_ID}/branch-staff/${item.id}`
    );
  };
        ...
        {loading || silentLoading ? (
          ...
          <AutoResizingGrid minWidth={284} gap={16} key="data">
            {state?.staffs.map((staff) => (
              <Link
                key={staff.id}
                href={`/staff/management/staff-branch/${BRANCH_ID}/branch-staff/${staff.id}`}
              >
                <UserCard
                  badge_color={
                    typeof staff.tier === "number" && staff.tier > 2
                      ? "gray"
                      : undefined
                  }
                  email={staff.email}
