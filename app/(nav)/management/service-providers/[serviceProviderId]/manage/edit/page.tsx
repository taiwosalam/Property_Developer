...
import { deleteServiceProvider, serviceProviderData as Mockdata, transformIndividualServiceProviderApiResponse } from "../data";
...
import type { IndividualServiceProvidersAPIResponse, ServiceProviderData } from "@/app/(nav)/accountant/management/service-providers/[serviceProviderId]/manage/types";
import { mockData } from "@/app/(nav)/management/landlord/data";
import { useRouter } from "next/navigation";

const EditServiceProvider = () => {
  const { serviceProviderId }  = useParams();
  const [providersData, setProvidersData] = useState<ServiceProviderData | null>(null)
  const { data, loading, silentLoading, isNetworkError, refetch } = useFetch<IndividualServiceProvidersAPIResponse>(`service-providers/${serviceProviderId}`);
        ...
        <FixedFooter className="flex justify-between items-center flex-wrap">
          <Modal>
            ...
            <ModalContent>
              ...
              action={async () => await deleteServiceProvider(serviceProviderId as string)}
